...
export async function GET() {
  ...
  try {
    const [wikiRes, crossrefRes, openAlexRes, numbersRes, archiveRes, iTunesRes] =
      await Promise.all([
        fetch(wikipediaUrl),
        fetch(crossrefUrl),
        fetch(openAlexUrl),
    ...
    const wikiArticles = wikiData.length >= 4
    ...
    const archiveVideos = archiveData.response?.docs?.map(doc => ({
